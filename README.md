...
## AdelBot Platform (Mobile + Web)

CDM includes a full **cloud platform** (`platform/`) that lets you run CDM and Claude Code **from your phone or a web browser**. It connects to GitHub repos, spins up Docker containers with CDM pre-installed, and streams the results back in real-time.
...
└──────────────┘     └──────────────┘     │  │ Claude Code / CDM│   │
      │                 │        │         │  └──────────────────┘   │
      │ OAuth           │ DB     │ Stream  │  /workspace (your repo) │
      ▼                 ▼        ▼         └──────────────────────────┘
  GitHub Auth      PostgreSQL  WebSocket           │ git push
                   + Redis     events              ▼
                                              GitHub Repos
```
...
|------|--------|----------|
| **Claude Code** | `claude -p` | Quick single-file tasks, bug fixes, simple features |
| **CDM Pipeline** | `cdm start` | Full features with 18 agents: requirements → architecture → implementation → review → testing → security → docs → deploy |

Users choose the mode when creating a session.

### Platform Components

| Component | Location | Stack |
|-----------|----------|-------|
| Backend API | `platform/backend/` | Python, FastAPI, PostgreSQL, Docker SDK |
| Android App | `platform/android/` | Kotlin, Jetpack Compose, Material 3 |
| Web Dashboard | `platform/web/` | React, TypeScript, Tailwind CSS |
| Dev Containers | `platform/docker/` | Docker images with Claude Code + CDM |
| Admin CLI | `platform/backend/manage.py` | Python CLI for user/billing management |
...
# Edit with your GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, ANTHROPIC_API_KEY
# Set ADMIN_GITHUB_USERNAMES=your-github-username

# 2. Build dev container images (with CDM pre-installed)
cd platform/docker/dev-environments && bash build.sh && cd ../../..

# 3. Start services
cd platform && docker compose up -d

# 4. Web dashboard at http://localhost:3000
...
# Admin management:
python -m backend.manage promote-admin <your-github-username>
python -m backend.manage list-users
python -m backend.manage stats
```

See [platform/README.md](platform/README.md) for full platform documentation.
