...
export function ReposPage() {
  ...
  const [executionMode, setExecutionMode] = useState<'claude' | 'cdm'>('claude')
  ...
  const createSession = async () => {
    ...
    setCreating(true)
    try {
      const session = await api.sessions.create({ repository_id: selectedRepo, task_description: task, branch: branch || undefined, execution_mode: mode })
          ...
          {selected ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={task} onChange={e => setTask(e.target.value)} rows={5} placeholder="Describe what you want done..." className="w-full mt-1 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm resize-none focus:outline-none focus:border-adel-500" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-2 block">Execution Mode</label>
                <div className="flex gap-2">
                  <button onClick={() => setExecutionMode('claude')}
                    className={`flex-1 p-3 rounded-lg border text-left text-sm transition-colors ${executionMode === 'claude' ? 'bg-adel-500/10 border-adel-500/30' : 'bg-gray-800 border-gray-700 hover:border-gray-600'}`}>
                    <p className="font-medium">Claude Code</p>
                    <p className="text-[11px] text-gray-500 mt-0.5">Quick single-agent mode</p>
                  </button>
                  <button onClick={() => setExecutionMode('cdm')}
                    className={`flex-1 p-3 rounded-lg border text-left text-sm transition-colors ${executionMode === 'cdm' ? 'bg-adel-500/10 border-adel-500/30' : 'bg-gray-800 border-gray-700 hover:border-gray-600'}`}>
                    <p className="font-medium">CDM Pipeline</p>
                    <p className="text-[11px] text-gray-500 mt-0.5">18-agent full pipeline</p>
                  </button>
                </div>
              </div>
